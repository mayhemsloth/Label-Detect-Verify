...
import inspect
def train_script_importable(
        weights: str = 'yolo7.pt',            # initial weights path
        cfg: str = '',                        # model.yaml path
        data: str = 'data/coco.yaml',         # data.yaml path
        ...
        notest: bool = False,                 # if True, test only final epoch
        noautoanchor: bool = False,           # if True, disable the autoanchor check
        evolve: bool = False,                 # evolve hyperparameters
        bucket: str = '',                     # gsutil bucket
        cache_images: bool = False,           # if True, cache images for faster training
        image_weights: bool = False,          # if True, use weighted image selection for training
        device = '',                          # cuda device, i.e. 0 or 0,1,2,3 or cpu
        multi_scale: bool = False,            # if True, vary image size by -+ 50% (as determined by hyperparameter.yaml file)
        single_cls: bool = False,             # if True, train multi-class data as single-class
        adam: bool = True,                    # if True, use torch.optim.Adam() optimizer
        sync_bn: bool = False,                # if True, use SynBatchNorm, only available in DDP mode
        local_rank: int = -1,                 # DDP parameter, do not modify
        workers: int = 8,                     # maximum number of dataloader workers
        project: str = 'runs/train',          # saves to project/name folder. Project is the core location where all training runs will be saved
        entity = None,                        # Weights & Biases entity
        name: str = 'exp',                    # saves to project/name folder. Name is the specific subfolder where this run will save to
        exist_ok: bool = False,               # if True, existing project/name ok, do not increment. If False, will increment name folder
        quad: bool = False,                   # if True, use quad dataloader (???)
        linear_lr: bool = False,              # if True, use linear learning rate. If False, uses default warmup and Cosine Decay
        label_smoothing: float = 0.0,         # label smooth epsilon
        upload_dataset: bool = False,         # if True, upload dataset as Weights & Biases artifact table
        bbox_interval: int = -1,              # Set bounding-box image logging interval for Weights & Biases
        save_period: int = -1,                # log after every "save_period" epoch
        artifact_alias: str = "latest",       # version of dataset artifact to be used
        freeze = [0],                         # list of Freeze layers: backbone of yolov7=50, first3=0 1 2'
        v5_metric: bool = False               # if True, assume maximum recall as 1.0 in AP calculation
    ...
    arg_names = inspect.getfullargspec(train_script_importable).args # Get the names of the function arguments
    _locals = locals()
